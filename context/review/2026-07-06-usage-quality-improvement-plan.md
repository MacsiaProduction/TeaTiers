...
| UX-P1-4 | **Single-CJK-char search never searches.** `MIN_CATALOG_QUERY_LEN = 2` gates both search VMs, so `茶` falls back to unfiltered browse — while the server's `strpos` path was *specifically built* for short/CJK queries. Special-case CJK codepoints (len ≥ 1). | `viewmodel/CatalogSearchModels.kt:52`, cf. `server/.../TeaSearchRepositoryImpl.kt:87-93` | **DONE (Batch 3)** — `isSearchableQuery()` bypasses the length floor for a single Han-script character; wired into both `AddTeaViewModel.catalogSearch` and `BrowseCatalogViewModel` (search + retry). |
| UX-P1-5 | **Enrichment gives up too early and never retries.** Poll budget ≈12s (8×1.5s) marks slower jobs FAILED permanently; `resumePending` runs once per process, so a tea stuck QUEUED from an offline add is never retried until full app restart. Longer/backoff poll ceiling + re-arm on reconnect. (Durable WorkManager version stays TRACKED as AND-P1-4/#70.6.) | `data/repository/TeaEnrichmentManager.kt:75-82,119-152` | **DONE (Batch 3)** — poll interval now doubles each attempt (capped 15s) over 10 attempts (~112s budget, up from ~12s); `resumePending` replaced its once-per-process `AtomicBoolean` with a 5-minute cooldown (`AtomicLong` + injectable clock) so a later board-open after connectivity likely returned can self-heal a stuck QUEUED tea, without a full app restart or a new permission/dependency. |
...
|---|---|:--:|---|:--:|
| UX-F-1 | **Wire the facets endpoint.** `GET /api/v1/teas/facets` exists on server and in `CatalogApi.kt:45` but has zero call sites — free win for type-filter counts in Browse and/or a small "your collection" stats card (counts per type/tier). | med | server done, client 0% | **DONE (Batch 5), scoped down** — `FacetsDto` has no counts (just the distinct type list), so "type-filter counts" wasn't literally deliverable; wired the endpoint for its actual purpose — type-filter chips on the Browse Catalog screen (mirrors My Teas' existing chip pattern), with `type` threaded through `search`/`browse` down to the server's already-supported `?type=` param. Skipped the separate "collection stats card" idea — that's local-data-only and doesn't need the facets endpoint; a distinct, unscoped feature beyond this finding's literal ask. |
| UX-F-2 | **My Teas sorting** (tier / date added / rating) — only a type filter + hardcoded alphabetical exists. Collection becomes hard to scan as it grows. | med | `viewmodel/MyTeasModels.kt:50` | **DONE (Batch 5), scoped down** — added a sort menu with `NAME` (existing default) and `TYPE` (grouped by `TeaType`'s own GB/T 30766-2014 category order, then name). "Tier" and "date added"/"rating" are NOT delivered: tier isn't well-defined for a cross-board shared tea (decisions.md #42 — can differ per board), and neither a timestamp nor a single rating scalar exists in the schema (`flavor` is 11-axis, not one number) — either needs a schema migration or a product decision, not a client-side sort. |
| UX-F-3 | **Fuzzy search of own teas.** Catalog search is trigram-fuzzy but personal-collection search is plain `.contains()` — a typo silently returns nothing. Client-side normalized/fuzzy match is enough at personal scale. | med | `viewmodel/MyTeasViewModel.kt:41-55` | **DONE (Batch 5)** — a hand-rolled Levenshtein check (no new dependency) falls back per-word when the exact substring match misses, for a 3+ character query (below that, CJK single-char tokens and short prefixes stay exact-only per `isSearchableQuery`'s reasoning). |
| UX-F-4 | **"Showing top N matches" hint** for typed search — fuzzy results are capped at one page by design (no cursor), but the UI gives no signal the list was truncated. One string + one condition. | low | `viewmodel/BrowseCatalogViewModel.kt:139-146` | **DONE (Batch 3)** — `Loaded.truncated` flags a full-page (uncertain-completeness) search result; the browse screen shows a hint row above the results. |
