...
| **P0-1 (2026-06-22)** | **Production-row UUID reconciliation** — prod is still pre-V7; deploying V7 assigns *random* UUIDs to the existing 100 rows and the seeder skips them, so a later rebuild yields the *frozen* UUIDs → orphaned clients | C1 | **DONE (code) — operator must rehearse+deploy** | V11 Java migration `db.migration.V11__reconcile_seed_public_ids` + testable `SeedPublicIdReconciler` (matches existing rows to the seed by `dedup_key`, rewrites public_id + legacy map FK-safely, fails on collision, idempotent, no-op on blank DB) + `SeedPublicIdReconcileIT`. **Owner action remains:** rehearse V6→current on a prod-like backup + deploy by verified digest, then probe numeric/UUID endpoints |
...
| FND-P1-4 / SCR-P1-2 | Evidence/input validation incomplete; unknown `type`→`OTHER`; country not ISO; region free-text; URL/host/evidence; lenient deserialization | **OPEN — P0 before scraper** | strict DTO/schema + URI/host/SSRF validation + ISO-3166 country + #138 region QID queue (or reject region) + persist `raw_evidence` proof chain + re-run facts guard at apply against the exact revision |
