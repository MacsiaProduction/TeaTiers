...
**Current state (DONE 2026-06-23 — migration-test harness landed):** `androidx.room:room-testing` + `robolectric` (4.15.1) + `junit-vintage-engine` are wired into the **unit-test** set. `TeaDatabaseMigrationTest` (`src/test`, `@RunWith(AndroidJUnit4)` → Robolectric on the JVM) has a v6 `createsAndValidates` baseline plus `robolectric_enforcesForeignKeysAndCascade`, which **proves Robolectric's SQLite enforces FK CASCADE + constraints** — retiring the Q8 "Robolectric can mock FK incorrectly" caveat. Schemas are exposed via `sourceSets["debug"].assets.srcDir("schemas")` + `unitTests.isIncludeAndroidResources = true`. The migration test runs inside `./gradlew check`, so both `ci.yml` (`app` job) and `release.yml` (`Full check before release`) gate on it — no emulator, no separate job (the old `app-instrumented` emulator job was removed). The `app/schemas` diff gate is live on the `app` job.

**Test infra:**
1. **DONE (2026-06-23):** `room-testing` + `robolectric` + `junit-vintage-engine` are in the **unit-test** set (not androidTest); schemas wired via `sourceSets["debug"].assets` + `unitTests.isIncludeAndroidResources = true` so `MigrationTestHelper` (under Robolectric) finds `6.json`/`7.json`.
2. Build once → KSP writes `app/schemas/.../7.json` → **commit it** (the diff contract).
3. **`MigrationTeaSampleSplitTest` (JVM/Robolectric, in `src/test` beside `TeaDatabaseMigrationTest`)** — Robolectric's SQLite enforces real FK CASCADE/constraint semantics (proven by `robolectric_enforcesForeignKeysAndCascade`), so no emulator is needed:
...
**CI gate (DONE 2026-06-23):** the migration test runs in `./gradlew check` (JVM), which both `ci.yml`'s `app` job and `release.yml`'s pre-release check already run — so the gate needs no separate emulator job (P1-4 is fixed: release now runs `check`, and `check` includes the migration test). The `git diff --exit-code app/schemas` gate is live on the `app` job, so an uncommitted `7.json` fails the build. The `MigrationTeaSampleSplitTest` + repo-smoke cases above are the remaining v7-specific work and slot straight into this harness. **Acceptance (P0-1 §9):** a DB created by v0.1.0 (v6) upgrades to v7 with every record + photo reference preserved, using the checked-in `6.json`.
